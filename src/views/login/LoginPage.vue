<script setup>
import { uesLoginService, useRegisterService } from '@/api/user'
...
const rules = ({
  username: [
    ...
    { min: 3, max: 10, message: '用户名必须是3-10为字符', trigger: 'blur' },
  ],
  password: [
    { required: true, message: '请输入密码', trigger: 'blur' },
    { pattern: /^.{6,15}$/, message: '密码必须是6-15的非空字符', trigger: 'blur' },
  ],
  repassword: [
    { required: true, message: '请输入密码', trigger: 'blur' },
    { pattern: /^.{6,15}$/, message: '密码必须是6-15的非空字符', trigger: 'blur' },
...
const login = async () => {
  await form.value.validate()
  const res = await uesLoginService(formModel.value)
